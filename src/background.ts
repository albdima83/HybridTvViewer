...
const allResourceTypes: chrome.declarativeNetRequest.ResourceType[] =
...
const DEFAULT_CHANNEL: Channel = channels[0].channel;
const ALL_CHANNELS = channels.map((i) => i.channel);
...
function onBeforeNavigate(
  ...
  if (isMainFrame && tabState.url && tabState.url !== details.url) {
    ...
    console.log(
      `[onBeforeNavigate] Resetting plugin state due to a URL change.`,
    );
...
function onHeadersReceived(
  ...
  if (hbbtvHeader) {
    console.log(
      `[onHeadersReceived] HbbTv header found "${hbbtvHeader.value}".`,
    );
    ...
    await Promise.all([
      chrome.scripting.executeScript({
        ...
        args: [
          ...
          tabState.channelList || ALL_CHANNELS,
        ],
  ...
  switch (msg.type) {
    case MessageType.GetPolyfillBaseConfig:
      response = {
        ...
        channelList: tabState.channelList || ALL_CHANNELS,
      };
      break;
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  onMessage(msg, sender, sendResponse).catch((err) =>
...
const setupPolyfillJs = (
  ...
  window.HBBTV_POLYFILL_NS.channelList = channelList;
};
